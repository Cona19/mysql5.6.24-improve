...
#        handle  -lpthread -lm -lcrypt -ldl  and other strings inserted that might contain
...
my $socket  = '/var/run/mysqld/mysqld.socket';
my $version = '5.6.24';
...
my $flags;
$flags->{libs} =
  [@ldflags,@lib_opts,'',' -lpthread -lm -ldl ','',''];
$flags->{libs_r} =
  [@ldflags,@lib_r_opts,'',' -lpthread -lm -lcrypt -ldl ',''];
$flags->{embedded_libs} =
  [@ldflags,@lib_e_opts,'','',' -lpthread -lm -lcrypt -ldl ','',''];
